...
var
  iNum1, iNum2, iUserAnswer, iAnswer, K, Y, T, L, X, U, iBossfight, iRandomSound, I : Integer;
  sUserAnswer : string;
  bWrongChar : Boolean;
const
//The '-' is for negative values.
  VALIDINPUT = ['0'..'9', '-'];
begin
  Randomize;
  ...
  sUserAnswer := InputBox('Hard Question', IntToStr(iNum1) + ' ' + cQuestionDisplaySymbol + ' ' + IntToStr(iNum2), '');

  //Checks if user only gave numbers or not
  for I := 1 to Length(sUserAnswer) do
  begin
  if NOT (sUserAnswer[I] IN VALIDINPUT) then
      bWrongChar := True;
  end;

  //Checks if the user gave no answer
  if Length(sUserAnswer) = 0 then
    bWrongChar := True;
  ...
  begin
    iUserAnswer := StrToInt(sUserAnswer);
...
begin
  ...
  imgMonster.Picture := imgNone.Picture;
