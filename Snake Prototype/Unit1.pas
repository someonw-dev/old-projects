...
uses
  ...
  Vcl.Imaging.jpeg, Vcl.Samples.Spin, Vcl.Imaging.pngimage;
...
var
  ...
  iMovement, iCounter, iHeadPosX, iHeadPosY, iTailPosX, iTailPosY, iTickrate, O, W, A, S, iBaseHeight, iPosModifier, iTitleDistance : Integer;
  bDeletePrev, bPlacedFruit, bDoVisualFix, bPauseUndoFix, bDisablePgc, bCheck, bClose, bPaused, bChangeCharacter, bDead : Boolean;
  sColour : string;
...
begin
  case Key of
    'w' : begin
            if (arrMoves[1] = 3) AND NOT (iCounter = 1) then
              iMovement := 3
            else
              iMovement := 1;
          end;
    'a' : begin
            if (arrMoves[1] = 4) AND NOT (iCounter = 1) then
              iMovement := 4
            else
              iMovement := 2;
          end;
    's' : begin
            if (arrMoves[1] = 1) AND NOT (iCounter = 1) then
              iMovement := 1
            else
              iMovement := 3;
          end;
    'd' : begin
            if (arrMoves[1] = 2) AND NOT (iCounter = 1) then
              iMovement := 2
            else
              iMovement := 4;
          end;
...
var
  I, L, iRanX, iRanY, iPrevX, iPrevY, iScore : Integer;
  bPlacedFruit : Boolean;
begin
  iPrevX := iHeadPosX;
  iPrevY := iHeadPosY;
